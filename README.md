...
The guards are idiots. They have an algorithm, but it only sometimes involves chasing you. Nevertheless, you need to learn their algorithm because they can carry gold, and you need to get it from them. In some levels, the gold is hidden in places that you can't get to, but you can kill the guards (by burying them and letting the ground swallow them) to make them respawn, sometimes in the places where you need them.

Also, you fall faster than they do. Sometimes, that's important.
...
This website has none of the original levels, only the ones that I designed, and the game engine is rewritten from scratch. It was inspired by [LodeRunner_TotalRecall](https://github.com/SimonHung/LodeRunner_TotalRecall) by Simon Hung, which has no license, but I didn't copy any code from it. The pixel art on this site is also original.
